...
# stable GD-n id and a trailing `<!-- gd: ... -->` metadata comment. ROADMAP.md is
...
lint() {
	...
	END {
		for (i = 1; i <= max; i++)
			if (!("GD-" i in seen)) err("GD-" i " is missing — ids run 1..N with no gaps; retire an item with status done, never by deleting it")
...
issue_body() {
	awk -v want="$1" -F'\t' \
		...
		-v repo_blob="https://github.com/Allan-Nava/galera-doctor/blob/main" '
	...
	$1 == "I" && $3 == want {
		...
		print ""
		print "`BACKLOG.md` is the single source of truth: it carries the stable `GD-n` id that commits and the CHANGELOG reference, and [ROADMAP.md](" repo_blob "/ROADMAP.md) is generated from it. This issue is a view of that item, kept in step by `scripts/backlog.sh issues --apply`, so closing it means ticking the item in the backlog and regenerating the roadmap in the same commit."
	}
...
# GD-63 is literally named `--profile apple|dash-if|none`. Splitting on it truncated
...
# titled "GD-n — " with nothing after the dash: `ver` is empty for an open item, so
...
issues_cmd() {
	...
	while [ $# -gt 0 ]; do
		...
		--title)
			...
			title_of="${1:-}"
			[ -n "$title_of" ] || {
				echo "backlog.sh: --title needs an GD-n id" >&2
				exit 2
			}
			;;
		--body)
			...
			body_of="${1:-}"
			[ -n "$body_of" ] || {
				echo "backlog.sh: --body needs an GD-n id" >&2
				exit 2
			}
			;;
		*)
