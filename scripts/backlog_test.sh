...
# row in three the first time an item title contained a `|` (GD-63): the
...
- [x] **GD-1 — Shipped and high**: nothing more to do here.
  <!-- gd: prio=high size=S labels=collect ver=0.1.0 -->
- [ ] **GD-2 — `--profile apple|dash-if|none`**: a pipe is legal in a title, and
  GD-63 is named exactly like this.
  <!-- gd: prio=med size=M labels=cli -->

## M2 — Later things <!-- ms: target=v0.2.0 phase=later -->

- [ ] **GD-3 — An em dash — right here**: it has to survive the round trip.
  <!-- gd: prio=low size=L labels=docs,tests -->
- [ ] **GD-4 — Open and high, listed after a low one**: priority decides the
...
checks=$((checks + 1))
if [ "$(grep -c '^| \*\*GD-2\*\*' "$tmp/ROADMAP.md" 2>/dev/null || grep -c 'GD-2' "$tmp/ROADMAP.md")" -ge 1 ]; then
	echo "ok   the row carrying a pipe is still one row"
else
	fail "GD-2 is missing from the roadmap entirely"
fi

# Every row of one markdown table has the same number of cells, and an escaped
# pipe is not one of them. This is the assertion GD-63 would have failed: the
...
checks=$((checks + 1))
if [ "$(grep -n 'GD-4' "$tmp/ROADMAP.md" | head -1 | cut -d: -f1)" \
	...
	fail "GD-4 (high) is listed after GD-3 (low)"
fi
...
assert_contains "next leads with the highest priority open item" "GD-4" "$tmp/next.txt"
assert_absent "next does not offer work that is already done" "GD-1" "$tmp/next.txt"
checks=$((checks + 1))
...
# An id used twice: two commits reference GD-2 and mean different work.
printf '%s\n%s\n' "$head" '- [ ] **GD-1 — One**: a.
  <!-- gd: prio=med size=S labels=cli -->
- [ ] **GD-1 — Two**: b.
  <!-- gd: prio=med size=S labels=cli -->' >"$tmp/dup.md"
assert_fails "a duplicate id is rejected" "$tmp/dup.md" "GD-1"
...
printf '%s\n%s\n' "$head" '- [ ] **GD-1 — One**: a.
  <!-- gd: prio=med size=S labels=cli -->
- [ ] **GD-3 — Three**: the gap where GD-2 was.
  <!-- gd: prio=med size=S labels=cli -->' >"$tmp/gap.md"
assert_fails "a gap in the sequence is rejected" "$tmp/gap.md" "GD-2"
...
printf '%s\n%s\n' "$head" '- [ ] **GD-1 — One**: a.
  <!-- gd: prio=urgent size=S labels=cli -->' >"$tmp/prio.md"
assert_fails "an unknown priority is rejected" "$tmp/prio.md" "prio"

printf '%s\n%s\n' "$head" '- [ ] **GD-1 — One**: a.
  <!-- gd: prio=med size=XXL labels=cli -->' >"$tmp/size.md"
...
printf '%s\n%s\n' "$head" '- [ ] **GD-1 — One**: a.' >"$tmp/nometa.md"
assert_fails "an item with no metadata is rejected" "$tmp/nometa.md" "GD-1"
...
printf '%s\n%s\n' "$head" '- [x] **GD-1 — One**: shipped, apparently.
  <!-- gd: prio=med size=S labels=cli -->' >"$tmp/nover.md"
...
" '- [ ] **GD-1 — One**: still open.
  <!-- gd: prio=med size=S labels=cli -->' >"$tmp/phase.md"
